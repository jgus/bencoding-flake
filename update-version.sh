...
# Pins pin.nix to a specific (or the latest) release of `bencoding` from PyPI and re-validates the source hash. Run from the flake root:
#
...
if [[ $# -ge 1 && -n "${1}" ]]; then
  ...
  echo "Querying PyPI for latest release of bencoding..."
  new_version=$(curl -sSfL https://pypi.org/pypi/bencoding/json | jq -r '.info.version')
fi
cur_version=$(nix eval --raw --file "${pin}" version)
cur_hash=$(nix eval --raw --file "${pin}" hash)

echo "Computing source hash for ${new_version}..."
url="https://files.pythonhosted.org/packages/source/b/bencoding/bencoding-${new_version}.tar.gz"
...
nix build --option post-build-hook "" "${FLAKE_ROOT}#bencoding" --no-link
